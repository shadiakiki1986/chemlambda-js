...
var jsExamplesOpt = [
  {"lambda": "λu.x", "javascript": "u=>x", "default": true},
  {"lambda": "λn.n(λu.x)", "javascript": "n=> n (u=>x)"},
  {"lambda": "three := λf.λx.f(f(f x))", "javascript": "f => x => f(f(f(x)))"},
  {"lambda": "PRED := λn.λf.λx.n (λg.λh.h (g f)) (λu.x) (λu.u)", "javascript": `n => f => x => {
        var L4 = u1 => x
  ...
  {"lambda": "PRED(three)", "javascript": `PRED3 => {
...
var app = new Vue({
  ...
  computed: {
    ...
    "dotFromLambda": function () {
      if(this.jsExSelected.javascript == "") { return }
      
      var dot_in = this.jsExSelected.javascript;
  ...
  watch: {
    ...
    "dotOut": function() {
      console.log("watching dotout")
      var lambda_dot = this.dotOut; // document.getElementById("dot_out").innerHTML;
